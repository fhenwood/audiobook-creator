...
import shlex
import json
from audiobook.utils.shell_commands import run_shell_command_secure, validate_file_path_allowlist
...
def concatenate_audio_files_with_ffmpeg(audio_files: list, output_path: str):
    """
    Concatenate a list of audio files into a single output file using FFmpeg.
    
    This is a simpler alternative to assemble_chapter_with_ffmpeg that takes
    a list of audio file paths directly instead of line indices.
    
    Args:
        audio_files (list): List of audio file paths to concatenate
        output_path (str): Path where the concatenated audio file will be created
    """
    if not audio_files:
        return
